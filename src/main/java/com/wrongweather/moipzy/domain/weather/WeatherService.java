...
public class WeatherService {
    ...
    public List<Integer> getWeatherInfo(String date) {
        ...
        try {
            ...
            log.info("temperature time: {}", (endTime - startTime));
    ...
    @Async
    //최저기온, 최고기온 순으로 return
    public CompletableFuture<List<Integer>> getWeatherInfoAsync(String date) {
        //String resultDate = convertDate(date);
        long startTime = System.currentTimeMillis();
        String resultDate = convertDate("today");
        ...
        try {
            ...
            return CompletableFuture.completedFuture(Arrays.asList(minTemperature, maxTemperature));
        } catch(Exception e){
            System.out.println("날씨 정보 호출 실패");
            e.printStackTrace();
        }
